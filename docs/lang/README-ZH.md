...
> 🌐 提供其他语言版本： [English](../../README.md) | [Español](README-ES.md)

---

一个 Visual Studio Code 扩展，可使用 **免费的 Google 翻译 API** 自动生成多语言文档文件（`README.md` 和 `CHANGELOG.md`） - 无需 API 密钥。
...
- ⚙️ **自动变更日志管理** — 检测 `CHANGELOG.md`，将变更日志部分添加到 `README.md`（如果缺失），然后进行翻译。
...
- 💬 向每个生成的自述文件添加语言切换器块（e.g., `- 🧠 使用内置 Google 翻译 — 无需帐户或自定义 API 密钥。
...
|自动设置变更日志 | `multi-doc-translator.autoSetupChangelog` | - |如果缺少，请将变更日志部分添加到 README.md |
...
您还可以将翻译器用作独立的命令行界面 (CLI) 应用程序，而无需打开 VS Code！
...
报告有关 [GitHub Issues Page](https://github.com/fatonyahmadfauzi/MultiDoc-Translator/issues) 的任何问题或建议。

---

## 🧾 变更日志

在 [CHANGELOG.md](CHANGELOG.md) 文件中查看每个版本的所有显着更改。
📦 您还可以直接在 [GitHub Releases page](https://github.com/fatonyahmadfauzi/MultiDoc-Translator/releases) 上查看发行说明。
